...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        def batcher(params, batch, lang="en"):
            ...
            #    batch['is_amr'] = torch.ones(batch_size).bool()
        ...
        if self.args.multilingual:
            ...
            metrics = {}
            metrics["eval_msts_spearman"] = results_semeval17["SemEval17.all"]['spearman']['mean'] 
            for i in results_semeval17:
                if i.endswith('all'):
                    continue
                metrics["eval_"+i.split('.')[4]+"_spearman"] = results_semeval17[i]['spearman'].correlation * 100
        else:
            ...
            # results = se.eval(tasks)
            # for task in tasks:
            #     metrics[f'eval_test_{task}'] = results[task]['test']['spearman'].correlation * 100
            # use_amr = self.use_amr
            # self.use_amr = False
            # results = se.eval(tasks)
            # for task in tasks:
            #     metrics[f'eval_old_test_{task}'] = results[task]['test']['spearman'].correlation * 100
            # self.use_amr = use_amr
        self.log(metrics)
        return metrics
