...
class KFoldClassifier(object):
    ...
    def run(self):
        ...
        if isinstance(testaccuracy, collections.Mapping):
            testaccuracy = {key: round(100*val, 2) for key, val in testaccuracy.items()}
        else:
            testaccuracy = round(100*testaccuracy, 2)

        return devaccuracy, testaccuracy, clf.predict(self.test['X'])
...
class SplitClassifier(object):
    ...
    """
    def __init__(self, X, y, config, multi_dev=False, combine_dev=True):
        self.X = X
        self.y = y
        self.nclasses = config['nclasses']
    ...
    def run(self):
        ...
        if self.usepytorch:
            ...
            # TODO: Find a hack for reducing nb epoches in SNLI
            devaccuracy = clf.fit(self.X['train'], self.y['train'],
                    validation_data=(self.X['valid'], self.y['valid']))
        else:
            clf = LogisticRegression(C=optreg, random_state=self.seed)
            clf.fit(self.X['train'], self.y['train'])
