...
class MLDocEval(object):
    def __init__(self, task_path, seed=1111, zero_shot=False):
        self.seed = seed
        self.zero_shot = zero_shot

        self.nclasses = 4
        logging.debug('***** Transfer task : MLDoc classification (zero-shot = {}) *****\n\n'.format(zero_shot))
        
        self.sst_data = {}
        self.langs = ['de','en','es','fr','it','ja','ru','zh']
        if not zero_shot:
            for lang in self.langs:
                self.sst_data['train.{}'.format(lang)] = self.loadFile(os.path.join(task_path, 'train.{}.txt'.format(lang)))
        else:
            self.sst_data['train.en'] = self.loadFile(os.path.join(task_path, 'train.en.txt'))
        for lang in self.langs:
            self.sst_data['dev.{}'.format(lang)] = self.loadFile(os.path.join(task_path, 'dev.{}.txt'.format(lang)))
            self.sst_data['test.{}'.format(lang)] = self.loadFile(os.path.join(task_path, 'test.{}.txt'.format(lang)))
...
#        return prepare(params, samples)
    ...
    def loadFile(self, fpath):
        ...
        with io.open(fpath, 'r', encoding='utf-8') as f:
            for line in f:
                tgt2idx = {'CCAT': 0, 'ECAT': 1, 'GCAT': 2, 'MCAT': 3}
                sample = line.strip().split('\t', 1)
                assert sample[0] in tgt2idx, sample
                sst_data['y'].append(tgt2idx[sample[0]])
                sst_data['X'].append(sample[1].strip().split())
