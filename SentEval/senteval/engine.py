...
from senteval.cls import CLSEval
from senteval.mldoc import MLDocEval
from senteval.pawsx import PAWSXEval
from senteval.xnli import XNLIEval
from senteval.marc import MARCEval
from senteval.qam import QAMEval
...
class SE(object):
    def __init__(self, params, batcher, prepare=None):
        ...
        self.list_tasks = ['CR', 'MR', 'MPQA', 'SUBJ', 'SST2', 'SST5', 'TREC', 'MRPC',
                           ...
                           'MLDoc_m', 'CLS_m', 'MARC_m',
                           'MLDoc_0', 'XNLI_0', 'PAWS-X_0', 'CLS_0', 'MARC_0', 'QAM_0']
    ...
    def eval(self, name):
        ...
        # ML Transfer Tasks
            ...
            self.evaluation = CLSEval(tpath + '/downstream/ml_transfer/CLS', seed=self.params.seed, zero_shot=False)
        elif name == 'CLS_0':
            self.evaluation = CLSEval(tpath + '/downstream/ml_transfer/CLS', seed=self.params.seed, zero_shot=True)
        elif name == 'MLDoc_m':
            self.evaluation = MLDocEval(tpath + '/downstream/ml_transfer/MLDoc', seed=self.params.seed, zero_shot=False)
        elif name == 'MLDoc_0':
            self.evaluation = MLDocEval(tpath + '/downstream/ml_transfer/MLDoc', seed=self.params.seed, zero_shot=True)
        elif name == 'PAWS-X_0':
            self.evaluation = PAWSXEval(tpath + '/downstream/ml_transfer/PAWSX', seed=self.params.seed)
        elif name == 'XNLI_0':
            self.evaluation = XNLIEval(tpath + '/downstream/ml_transfer/XNLI', seed=self.params.seed)
        elif name == 'MARC_m':
            self.evaluation = MARCEval(tpath + '/downstream/ml_transfer/MARC', seed=self.params.seed, zero_shot=False)
        elif name == 'MARC_0':
            self.evaluation = MARCEval(tpath + '/downstream/ml_transfer/MARC', seed=self.params.seed, zero_shot=True)
        elif name == 'QAM_0':
            self.evaluation = QAMEval(tpath + '/downstream/ml_transfer/QAM', seed=self.params.seed)
