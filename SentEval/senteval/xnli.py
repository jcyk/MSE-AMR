...
class XNLIEval(object):
    def __init__(self, task_path, seed=1111):
        logging.debug('***** Transfer task : XNLI *****\n\n')
        self.seed = seed
        self.langs = ["ar", "bg", "de", "el", "en", "es", "fr", "hi", "ru", "sw", "th", "tr", "ur", "vi", "zh"]
        train = self.loadFile(os.path.join(task_path, 'train.tsv'))
        self.sick_data = {'train.en': train}
        for lang in self.langs:
            self.sick_data['dev.{}'.format(lang)] = self.loadFile(os.path.join(task_path, 'dev-{}.tsv'.format(lang)))
            self.sick_data['test.{}'.format(lang)] = self.loadFile(os.path.join(task_path, 'test-{}.tsv'.format(lang)))
...
#        return prepare(params, samples)
    ...
    def run(self, params, batcher):
        sick_embed = {} #sick_embed = {'train': {}, 'dev': {}, 'test': {}}
        bsize = params.batch_size
        ...
        # Train
        trainA = sick_embed['train.en']['X_A']
        trainB = sick_embed['train.en']['X_B']
        trainF = np.c_[np.abs(trainA - trainB), trainA * trainB]
        trainY = np.array(self.sick_data['train.en']['y'])

        # Dev
        devF = {}
        devY = {}
        for lang in self.langs:
            devA = sick_embed['dev.{}'.format(lang)]['X_A']
            devB = sick_embed['dev.{}'.format(lang)]['X_B']
            devF[lang] = np.c_[np.abs(devA - devB), devA * devB]
            devY[lang] = np.array(self.sick_data['dev.{}'.format(lang)]['y'])

        # Test
        testF = {}
        testY = {}
        for lang in self.langs:
            testA = sick_embed['test.{}'.format(lang)]['X_A']
            testB = sick_embed['test.{}'.format(lang)]['X_B']
            testF[lang]  = np.c_[np.abs(testA - testB), testA * testB]
            testY[lang]  = np.array(self.sick_data['test.{}'.format(lang)]['y'])
        ...
        logging.debug(('\nDev acc : {0} Test acc : {1} ' 
            'for XNLI\n').format(devacc, ', '.join(['({}) {}'.format(k, v) for k, v in testacc.items()])))
#        for lang in self.langs:
