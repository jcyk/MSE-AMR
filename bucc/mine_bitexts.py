...
def knnGPU(x, y, k, mem=3*1024*1024*1024):
    dim = x.shape[1]
...
def mine(_src, _trg, _src_lang, _trg_lang, _mode, _margin, _src_embeddings, _trg_embeddings, _unify, _gpu, _verbose, _encoding='utf-8', _threshold=0.0, _neighborhood=4, _retrieval='max', _output=None):
...
#    args = parser.parse_args()
    ...
    args = argparse.Namespace(src=_src, trg=_trg, src_lang=_src_lang, trg_lang=_trg_lang, output=_output, threshold=_threshold, mode=_mode, margin=_margin, src_embeddings=_src_embeddings, trg_embeddings=_trg_embeddings, 
        unify=_unify, gpu=_gpu, verbose=_verbose, encoding=_encoding, neighborhood=_neighborhood, retrieval=_retrieval)
...
#    y = EmbedLoad(args.trg_embeddings, args.dim, verbose=args.verbose)
    ...
    in_memory_output = False
    
    if args.output is None:
        fout = io.StringIO()
        args.output = 'in-memory text buffer'
        in_memory_output = True
    else:
