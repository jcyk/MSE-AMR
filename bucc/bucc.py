...
def bucc(_src_lang, _trg_lang, _bucc_texts, _bucc_ids, _candidates, _verbose, _encoding='utf-8', _gold=None, _threshold=-1.0, _output=None):
    ...
    args = argparse.Namespace(src_lang=_src_lang, trg_lang=_trg_lang, bucc_texts=_bucc_texts, bucc_ids=_bucc_ids, candidates=_candidates, 
        verbose=_verbose, encoding=_encoding, gold=_gold, threshold=_threshold, output=_output)
...
#                # id2txt[src_id + '\t' + trg_id] = src + '\t' + trg
    ...
    candidate2score = {}
    for line in args.candidates.split('\n')[:-1]:
        score, src, trg = line.split('\t')
        score = float(score)
        src = src.strip()
        trg = trg.strip()
