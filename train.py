...
from amr import init_amr_vocabulary, reset_model_with_tokenizer
...
class ModelArguments:
    ...
    path_to_amr: str = field(
        default="amr/cache.txt",
        metadata={"help": "load amr file"}
    )
...
class OurTrainingArguments(TrainingArguments):
    ...
    multilingual: bool = field(
        default=False,
        metadata={"help": "we are training a multilingual model"}
    )
...
def main():
    ...
    if extension == "csv":
        import csv 
        datasets = load_dataset(extension, data_files=data_files, cache_dir=model_args.cache_dir, delimiter="\t" if "tsv" in data_args.train_file else ",", encoding='utf-8')
        #datasets = load_dataset(extension, data_files=data_files, cache_dir=model_args.cache_dir, delimiter="\t" if "tsv" in data_args.train_file else ",", quoting=csv.QUOTE_NONE, encoding='utf-8')
    else:
    ...
    if model_args.use_amr:
        if 'xlm-roberta' in model_args.model_name_or_path:
            INIT = '▁'
        elif 'roberta' in model_args.model_name_or_path:
            INIT = 'Ġ'
        else:
            INIT = ''
        old_enc_size = init_amr_vocabulary(tokenizer, INIT=INIT)
    ...
    if model_args.model_name_or_path:
        if 'roberta' in model_args.model_name_or_path:
            ...
            if model_args.use_amr:
                reset_model_with_tokenizer(model, tokenizer, old_enc_size)
    ...
    if model.update_moving_average(replace=True):
        logger.info("initialized target network")
    ...
    def prepare_features(examples):
        ...
        #is_amr = [sentence.split()[0] == '$$$' for sentence in sentences]
        #sent_features['is_amr'] = is_amr
    ...
    class OurDataCollatorWithPadding:
        ...
        def __call__(self, features: List[Dict[str, Union[List[int], List[List[int]], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
            special_keys = ['input_ids', 'is_amr', 'attention_mask', 'token_type_ids', 'mlm_input_ids', 'mlm_labels']
            bs = len(features)
            if bs > 0:
    ...
    trainer = CLTrainer(
        use_amr=model_args.use_amr,
        path_to_amr=model_args.path_to_amr,
        drop_parentheses=model_args.drop_parentheses,
        model=model,
        args=training_args,
    ...
    if training_args.do_eval:
        ...
        results = trainer.evaluate(eval_senteval_transfer= not model_args.use_amr)
